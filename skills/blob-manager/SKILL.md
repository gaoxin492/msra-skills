...
SAS URL 持久化在用户配置目录中：
```
~/.config/msra-skills/blob_sas.json
```
...
通过跳板机的 az CLI 自动生成，无需打开 Azure Storage Explorer：

```bash
# 在本地执行，通过跳板机生成 User Delegation SAS（7 天有效）
SAS=$(ssh msra "az storage container generate-sas \
  --account-name yifanyang \
  --name yifanyang \
  ...
  --as-user \
  -o tsv")
echo "https://yifanyang.blob.core.windows.net/yifanyang?${SAS}"
```

Claude 执行此命令后会自动更新 `blob_sas.json` 并验证连接。
...
- 用 **方式 A** 一条命令续期，或从 Azure Storage Explorer 手动重新生成
